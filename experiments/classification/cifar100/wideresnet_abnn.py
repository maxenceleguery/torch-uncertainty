...
from torch_uncertainty.optimization_procedures import (
    optim_cifar100_wideresnet28x10_abnn_finetuning,
)
...
if __name__ == "__main__":
    ...
    with ArgvContext(
        ...
        nets = [
            wideresnet28x10(
                in_channels=3, num_classes=dm.num_classes, style="Cifar"
            )
            for _ in range(num_emsembles)
        ]

        checkpoint = torch.load(
            str(root / "data/wr28x10_c100.ckpt")
        ...
        routines = [
            ClassificationSingle(
                ...
                optimization_procedure=optim_cifar100_wideresnet28x10_abnn_finetuning,
                **vars(args),
            )
        ...
        for i, routine in enumerate(routines):
            cli_main(routine, dm, root, f"logs/abnn/wr28x10_c100_{i}", args)
            p = Path()
            paths.append(
                next(
                    p.glob(
                        f"logs/abnn/wr28x10_c100_{i}/version_0/checkpoints/epoch*.ckpt"
                    )
                )
            )
