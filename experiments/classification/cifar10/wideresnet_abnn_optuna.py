...
if __name__ == "__main__":
    ...
    with ArgvContext(
        ...
        def get_weights_rp():
            ...
            return weights.to("cuda:1" if use_cuda else "cpu")

        def objective(trial):
            lr = trial.suggest_float("lr", 0.0001, 0.005)

            nets = [
                wideresnet28x10(in_channels=3, num_classes=10, style="Cifar")
                for _ in range(num_emsembles)
            ]

            checkpoint = torch.load(
                str(root / "data/wideresnet28x10_c10_3.ckpt")
            ...
            """
            shutil.rmtree("logs2/abnn")

            paths = []
            for i, routine in enumerate(routines):
                cli_main(routine, dm, root, f"logs2/abnn/wr28x10_c10_{i}", args)
                paths.append(
                    f"logs2/abnn/wr28x10_c10_{i}/version_0/checkpoints/epoch=0-step=782.ckpt"
                )
            ...
            trainer = pl.Trainer(False, False, accelerator="gpu", devices="1,")
