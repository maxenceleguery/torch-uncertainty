...
from torch_uncertainty.models.wideresnet.strange_wideresnet import _WideResNet
from torch_uncertainty.optimization_procedures import (
    optim_cifar10_wideresnet28x10_abnn_finetuning,
)
...
if __name__ == "__main__":
    ...
    with ArgvContext(
        ...
        nets = [
            _WideResNet(28, 10, 0.3, dm.num_classes)
            # wideresnet28x10(in_channels=3, num_classes=dm.num_classes, style="Cifar")
            for _ in range(num_emsembles)
        ]

        checkpoint = torch.load(
            str(root / "data/wr28x10_c10.ckpt")
        ...
        routines = [
            ClassificationSingle(
                ...
                optimization_procedure=optim_cifar10_wideresnet28x10_abnn_finetuning,
                **vars(args),
            )
        ...
        for i, routine in enumerate(routines):
            cli_main(routine, dm, root, f"logs/abnn/wr28x10_c10_{i}", args)
            p = Path()
            paths.append(
                next(
                    p.glob(
                        f"logs/abnn/wr28x10_c10_{i}/version_0/checkpoints/epoch*.ckpt"
                    )
                )
            )
